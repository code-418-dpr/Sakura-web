...
export const CSGOModifierGame: React.FC = () => {
  ...
  const [recentMaxMultiplier, setRecentMaxMultiplier] = React.useState<number>(GAME_CONFIG.maxMultiplier);
    ...
    gameTimerRef.current = setInterval(() => {
      setState(prev => {
        ...
        const newMultiplier = Math.min(
          prev.currentMultiplier + growthRate, 
  ...
  return (
    <Card className="w-full max-w-4xl bg-content1 overflow-visible">
      <CardBody className="p-6">
        <div className="flex flex-col gap-6">
          {/* Header */}
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-bold text-foreground">CS:GO Style Modifier Game</h1>
            <div className="flex items-center gap-2">
              <Icon icon="lucide:wallet" className="text-primary" />
              <span className="font-semibold text-lg">{formatCurrency(state.balance)}</span>
            </div>
          </div>
          
          {/* Game Area */}
          <div className="relative bg-content2 rounded-xl p-6 overflow-hidden">
            {/* Sakura petals animation */}
            <AnimatePresence>
              {state.isPlaying && petals}
            </AnimatePresence>
            
            {/* Rearranged layout - Graph and Multiplier side by side */}
            <div className="flex flex-col md:flex-row gap-6 mb-6">
              {/* Graph - Now on the left with square aspect ratio */}
              <div className="h-48 md:h-auto md:flex-1 w-full bg-content1 rounded-lg p-2 aspect-square">
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <YAxis 
                      domain={[0, state.currentMultiplier * 1.1]} 
                      hide 
                    />
                    <defs>
                      <linearGradient id="successGradient" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(var(--heroui-success-500))" stopOpacity={0.8}/>
                        <stop offset="95%" stopColor="hsl(var(--heroui-success-500))" stopOpacity={0.3}/>
                      </linearGradient>
                      <linearGradient id="dangerGradient" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="hsl(var(--heroui-danger-500))" stopOpacity={0.8}/>
                        <stop offset="95%" stopColor="hsl(var(--heroui-danger-500))" stopOpacity={0.3}/>
                      </linearGradient>
                    </defs>
                    <Line 
                      type="monotone" 
                      ...
                      fill={state.currentMultiplier >= 1 ? "url(#successGradient)" : "url(#dangerGradient)"}
              ...
              <div className="flex flex-col items-center justify-center relative z-20 md:w-1/3">
                <span className="text-sm text-default-600 mb-2">Current Multiplier</span>
                <motion.div
                ...
                <div className="w-full max-w-md mt-4">
                  <Progress 
                    ...
                    color={state.currentMultiplier > 2 ? "danger" : state.currentMultiplier > 1.5 ? "warning" : "primary"}
                    className="h-2"
                  ...
                  endContent={
                    <div className="flex gap-1">
                      <Button 
                        ...
                        onPress={() => setState(prev => ({ ...prev, betAmount: Math.floor(prev.balance / 2) }))}
                      >
                        1/2
                      </Button>
                      <Button 
                        ...
                        color="primary"
                        isDisabled={state.isPlaying}
                        onPress={() => setState(prev => ({ ...prev, betAmount: prev.balance }))}
                      >
                        Max
                      </Button>
                    </div>
              ...
              <div className="flex gap-4">
                ...
                <Tooltip content="Add funds">
                  <Button
                    ...
                    variant="flat"
                    isIconOnly
                    onPress={() => setState(prev => ({ ...prev, balance: prev.balance + 1000 }))}
                    isDisabled={state.isPlaying}
                  >
          ...
          <div>
            ...
            <div className="bg-content2 rounded-lg p-4 max-h-40 overflow-y-auto">
              {state.gameHistory.length === 0 ? (
                <p className="text-center text-default-500">No games played yet</p>
              ) : (
                <div className="space-y-2">
                  {state.gameHistory.map((game, index) => {
                    ...
                    return (
                      <div 
                        key={index} 
                        className={`flex justify-between items-center p-2 rounded-md ${getHistoryItemColor(game)}`}
                      >
                        <div className="flex items-center gap-2">
                          <Icon 
                            icon={game.win ? (game.multiplier >= 1 ? "lucide:check-circle" : "lucide:alert-circle") : "lucide:x-circle"} 
          ...
          <div className="text-sm text-default-500 mt-2">
            <p>
              Place your bet and try to cash out before the multiplier burns!
            </p>
          </div>
        </div>
      </CardBody>
